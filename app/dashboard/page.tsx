...
export default function DashboardPage() {
  ...
  const fetchData = React.useCallback(async () => {
    try {
      const [historyRes, libraryRes, unreadRes, downloadRes] = await Promise.all([
        // Continue Reading
        client.query({
          chapters: {
            __args: { orderBy: "LAST_READ_AT" as any, orderByType: "DESC" as any, first: 1 },
            nodes: {
              id: true,
              name: true,
              lastReadAt: true,
      ...
      setStats({
        ...
        downloading: downloadRes.downloadStatus?.queueSize || 0
      });
      ...
      <div className="flex flex-col gap-10 pb-20">
        ...
        <section className="flex flex-col gap-2">
           <div className="flex items-center gap-2 text-primary">
              ...
              <span className="text-sm font-black uppercase tracking-[0.3em]">Welcome Back</span>
           </div>
        ...
        {continueReading && !isLoading ? (
          <div 
            className="group relative h-80 w-full overflow-hidden rounded-3xl border border-white/5 bg-zinc-900 shadow-2xl transition-all hover:border-primary/30 cursor-pointer"
            onClick={() => router.push(`/manga/${continueReading.manga.id}/chapter/${continueReading.id}`)}
          >
            <img 
              src={getImageUrl(continueReading.manga.thumbnailUrl)!} 
              alt={continueReading.manga.title} 
              className="absolute inset-0 h-full w-full object-cover opacity-40 blur-[2px] transition-all duration-700 group-hover:scale-110 group-hover:opacity-60 group-hover:blur-0"
            />
            <div className="absolute inset-0 bg-gradient-to-t from-zinc-950 via-zinc-950/40 to-transparent" />
            
            <div className="absolute bottom-0 left-0 p-8 md:p-12 flex flex-col gap-4 w-full md:w-2/3">
              <div className="flex items-center gap-3">
                ...
                <span className="flex items-center gap-1.5 text-white/40 text-xs font-bold">
                  ...
                  Last read {new Date(parseInt(continueReading.lastReadAt)).toLocaleDateString()}
                </span>
              </div>
              <div className="space-y-1">
                <h2 className="text-3xl md:text-5xl font-black text-white leading-tight drop-shadow-sm font-heading">
              ...
              <div className="flex items-center gap-4 mt-2">
                <Button size="lg" className="rounded-2xl px-8 h-12 gap-3 shadow-xl shadow-primary/20 font-bold transition-transform active:scale-95">
                  <Play className="size-5 fill-current" /> Resume Now
                </Button>
                <Button variant="ghost" className="rounded-2xl h-12 text-white hover:bg-white/10 font-bold bg-white/5 backdrop-blur-md">
                  Details
                </Button>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 grid grid-cols-2 sm:grid-cols-3 gap-4">
            ...
            <StatCard 
              ...
              icon={<Download className="size-6 text-orange-400" />} 
              trend="In Queue"
            />
          </div>
          ...
          <div className="flex flex-col gap-4">
            <h3 className="text-xs font-black uppercase tracking-[0.2em] text-muted-foreground px-1">Navigation</h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="flex flex-col gap-6">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <h2 className="text-xl font-bold font-heading">Recent Updates</h2>
              </div>
              <Button variant="ghost" size="sm" className="text-muted-foreground gap-1 text-xs" onClick={() => router.push('/library')}>
                ...
                recentUpdates.map((manga) => (
                  ...
                  >
                    <div className="size-14 rounded-xl overflow-hidden shrink-0 shadow-lg">
                      <img src={getImageUrl(manga.thumbnailUrl)!} alt={manga.title} className="size-full object-cover" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <h4 className="font-bold text-foreground line-clamp-1 group-hover:text-primary transition-colors">{manga.title}</h4>
                      <p className="text-xs text-muted-foreground font-medium">{manga.unreadCount} new chapters available</p>
                    </div>
          ...
          <div className="flex flex-col gap-6">
             <div className="flex items-center gap-3">
                ...
                <h2 className="text-xl font-bold font-heading">Discovery Tip</h2>
              </div>
              <div className="h-full rounded-3xl bg-gradient-to-br from-primary/10 via-background to-background border border-primary/20 p-8 flex flex-col justify-between">
                <div className="space-y-4">
                  ...
                  <p className="text-muted-foreground font-medium">
                    Regularly checking the Discovery tab ensures you have the latest scrapers and security patches from the Keiyoushi repo.
                  </p>
                </div>
                <Button 
                  onClick={() => router.push('/browse')} 
...
function StatCard({ label, value, icon, trend }: { label: string, value: string | number, icon: React.ReactNode, trend: string }) {
  return (
    <div className="flex flex-col gap-4 p-6 rounded-3xl border border-white/5 bg-muted/5 hover:bg-muted/10 transition-all group shadow-sm">
      ...
      <div className="space-y-1">
        <div className="text-4xl font-black text-foreground drop-shadow-sm font-heading">{value}</div>
        <div className="text-xs text-muted-foreground font-bold">{trend}</div>
      </div>
    </div>
  );
}

function ActionButton({ label, icon, onClick, color }: { label: string, icon: React.ReactNode, onClick: () => void, color: string }) {
  return (
    <button 
      onClick={onClick}
      className={cn(
        "flex items-center justify-between p-5 rounded-2xl border transition-all hover:scale-[1.02] active:scale-95 group shadow-sm",
        color
      )}
    >
      <div className="flex items-center gap-4">
        ...
        <span className="font-bold text-sm tracking-tight uppercase tracking-wider">{label}</span>
      </div>
