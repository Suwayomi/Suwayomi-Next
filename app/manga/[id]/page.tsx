...
export default function MangaDetailPage() {
  ...
  const [data, setData] = React.useState<MangaDetail | null>(null);
  const [isLoading, setIsLoading] = React.useState(true);
  const [isDescriptionExpanded, setIsDescriptionExpanded] = React.useState(false);
  const [chaptersSort, setChaptersSort] = React.useState<"asc" | "desc">("desc");
  const [selectedChapterIds, setSelectedChapterIds] = React.useState<Set<number>>(new Set());
  ...
  const markPreviousAsRead = async (order: number) => {
    const chaptersToMark = data?.manga?.chapters.nodes
            ...
            <div className="flex flex-col flex-1 gap-6 pt-2">
              <div className="flex flex-col gap-3">
                <div className="flex items-center gap-2">
                  <span className="flex items-center gap-1 uppercase tracking-wider text-[10px] font-bold bg-primary/10 text-primary px-2 py-0.5 rounded border border-primary/20">
                    {manga.status}
                  </span>
                  <span className="flex items-center gap-1 text-[10px] font-bold text-muted-foreground uppercase tracking-widest px-1">
              ...
              <div className="flex flex-wrap gap-6 items-center bg-muted/20 w-fit px-6 py-3 rounded-2xl border border-border/40 font-mono">
                <div className="flex flex-col">
                  ...
                  <span className="text-xl font-bold text-primary">{manga.unreadCount}</span>
                </div>
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-xl font-bold">{manga.chapters.totalCount}</span>
                </div>
                {manga.inLibrary && (
                  <>
                    <Separator orientation="vertical" className="h-8 bg-border/40" />
              ...
              <div className="flex flex-wrap gap-2">
                {manga.genre.map((g) => (
                  <Badge key={g} variant="secondary" className="bg-muted hover:bg-muted/80 text-xs py-1 px-3 border-none">
                    {g}
                  </Badge>
                ))}
              </div>
              ...
              <div className="flex flex-wrap gap-4 pt-2">
                <Button
                  size="lg"
                  className="h-12 px-10 gap-2 shadow-xl shadow-primary/20 rounded-full font-bold"
                  onClick={() => {
                    if (manga.firstUnreadChapter) {
                      router.push(`/manga/${id}/chapter/${manga.firstUnreadChapter.id}`);
                    }
                  }}
                >
                  <Play className="size-4 fill-current" />
                  Read {manga.firstUnreadChapter?.name || "Now"}
                </Button>
                {!manga.inLibrary && (
                  <Button 
                    size="lg" 
                    variant="secondary" 
                    className="h-12 px-8 gap-2 border border-border/40 rounded-full font-bold"
                ...
                <Button size="lg" variant="outline" className="h-12 px-6 gap-2 border border-border/40 rounded-full font-bold">
        ...
        <div className="flex flex-col gap-12">
          ...
          <div className="flex flex-col gap-4 max-w-4xl">
            <div className="flex items-center gap-2 text-foreground font-bold">
              ...
              <h2 className="uppercase text-xs tracking-widest">Synopsis</h2>
            </div>
            <div className={cn(
            ...
            <div className="flex items-center justify-between sticky top-0 bg-background/80 backdrop-blur-md z-30 py-4 -mx-4 px-4">
              ...
              <div className="flex items-center gap-2">
                <Button 
                  variant="ghost" 
                  size="sm" 
                  onClick={() => setChaptersSort(chaptersSort === "asc" ? "desc" : "asc")}
              ...
              {chapters.map((chapter) => (
                <div 
                  ...
                  className={cn(
                    ...
                    selectedChapterIds.has(chapter.id) && "bg-primary/5 opacity-100 ring-1 ring-inset ring-primary/20"
                  )}
                  onClick={() => {
                    if (selectedChapterIds.size > 0) {
                      toggleChapterSelection(chapter.id);
                    } else {
                      router.push(`/manga/${id}/chapter/${chapter.id}`);
                    }
                  }}
                >
                  <div className="flex items-center gap-4 flex-1 min-w-0">
                    {selectedChapterIds.size > 0 && (
                      <div className={cn(
                        ...
                        selectedChapterIds.has(chapter.id) ? "bg-primary border-primary" : "bg-muted"
                      )}>
                        {selectedChapterIds.has(chapter.id) && <Check className="size-3 text-primary-foreground stroke-[3px]" />}
                      </div>
                    )}
                    
                    <div className="flex flex-col gap-1 min-w-0">
                      <div className="flex items-center gap-2">
                        {chapter.isRead && !selectedChapterIds.has(chapter.id) && <CheckCircle2 className="size-4 text-primary" />}
                        <span className={cn(
                          "font-bold truncate text-foreground",
                          chapter.isRead && !selectedChapterIds.has(chapter.id) && "text-muted-foreground font-medium"
                        )}>
                          {chapter.name}
                        </span>
                      </div>
                      <div className="flex items-center gap-3 text-[11px] text-muted-foreground font-medium">
                        ...
                        {chapter.scanlator && (
                          <>
                            ...
                            <span className="truncate max-w-[150px]">{chapter.scanlator}</span>
                          </>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="flex items-center gap-3 ml-4">
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-56" onClick={(e) => e.stopPropagation()}>
                        <DropdownMenuItem onClick={() => toggleChapterSelection(chapter.id)}>
                        ...
                        {!chapter.isDownloaded ? (
                          <DropdownMenuItem onClick={() => downloadChapters([chapter.id])}>
                            <Download className="mr-2 size-4" />
                            <span>Download</span>
                          </DropdownMenuItem>
                        ) : (
                          <DropdownMenuItem onClick={() => deleteDownloadedChapters([chapter.id])} className="text-destructive">
                        ...
                        <DropdownMenuItem onClick={() => markAsRead([chapter.id], !chapter.isRead)}>
                          <History className="mr-2 size-4" />
                          <span>Mark as {chapter.isRead ? "unread" : "read"}</span>
                        </DropdownMenuItem>
                        
                        <DropdownMenuItem onClick={() => markPreviousAsRead(chapter.sourceOrder)}>
          ...
          <div className="flex items-center gap-4 bg-zinc-900/90 backdrop-blur-md border border-white/10 px-6 py-3 rounded-full shadow-2xl ring-1 ring-black/10">
            ...
            <div className="flex items-center gap-1">
              <Button 
                ...
                onClick={() => downloadChapters(Array.from(selectedChapterIds))}
              >
              ...
              <Button 
                ...
                size="sm" 
                className="h-9 gap-2 px-3 text-white hover:bg-white/10"
                onClick={() => markAsRead(Array.from(selectedChapterIds), true)}
              >
              ...
              <Button 
                ...
                onClick={() => deleteDownloadedChapters(Array.from(selectedChapterIds))}
              >
