...
export type ReadingMode = "single-page" | "double-page" | "continuous-vertical" | "continuous-horizontal" | "webtoon";
...
const ReaderSettingsContext = React.createContext<ReaderSettingsContextType | undefined>(undefined);
...
export function ReaderSettingsProvider({ children }: { children: React.ReactNode }) {
    ...
    const updateSettings = React.useCallback((updates: Partial<ReaderSettings>) => {
        setSettings((prev) => {
    ...
    const value = React.useMemo(() => ({
        ...settings,
        updateSettings,
        setReadingMode: (mode: ReadingMode) => updateSettings({ readingMode: mode }),
        setReadingDirection: (dir: ReadingDirection) => updateSettings({ readingDirection: dir }),
        setTapZone: (zone: TapZone) => updateSettings({ tapZone: zone }),
        setInvertTapZone: (invert: InvertTapZone) => updateSettings({ invertTapZone: invert }),
        setScaleType: (scale: ScaleType) => updateSettings({ scaleType: scale }),
        setHudType: (type: HudType) => updateSettings({ hudType: type }),
        setHudOrientation: (orient: HudOrientation) => updateSettings({ hudOrientation: orient }),
        setPageGap: (gap: number) => updateSettings({ pageGap: gap }),
        setBackground: (bg: "black" | "zinc") => updateSettings({ background: bg }),
